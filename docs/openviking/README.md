...
本仓库把 OpenViking 状态分成两类：

- 可提交：`docs/openviking/`、`scripts/openviking-*.sh`、`scripts/openviking-local-embedding-server.mjs`、以及 `.openviking-data/viking/default/resources/wiki/` 和 `.openviking-data/vectordb/context/` 这类已生成资源与本地向量索引。
- 不提交：`.openviking-local/`、`.openviking-data/_system/`、`.openviking-data/temp/`、`.openviking-data/usage_audit.sqlite3*`、pid、LOCK、LOG 等私有配置、venv、队列、审计、临时上传和运行锁。

换设备后，如果 `.openviking-data` 已随 git 拉取，只需要安装 OpenViking、重新生成 `.openviking-local/ov.conf` / `ovcli.conf` 并启动服务；除非 manifest 或核心笔记发生变化，否则不必重新摄入整批资源。
